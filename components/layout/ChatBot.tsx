...
let msgId = 0
const newMsg = (type: MsgType, text: string, meta: Partial<Msg> = {}): Msg => ({ id: ++msgId, type, text, ...meta })
...
function MessageBubble({ msg }: { msg: Msg }) {
  ...
  if (msg.type === 'bot') {
    return (
      <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} className="flex items-end gap-2">
        <BotAvatar />
...
export default function ChatBot() {
  ...
  const pushNode = useCallback((key: string, userText?: string) => {
    const node = TREE[key]
    if (!node) return
          ...
          <AnimatePresence mode="wait">
            {open ? (
              ...
              <motion.span
                ...
                transition={{ duration: 0.18 }}
              >
                <svg className="h-7 w-7" fill="none" viewBox="0 0 28 28" stroke="currentColor" strokeWidth={1.8}>
                  <path
                    strokeLinecap="round"
        ...
        {open && (
          ...
          >
            <div className="flex flex-shrink-0 items-center gap-3 bg-gradient-to-r from-sky-600 to-indigo-600 px-4 py-3.5">
              ...
              <button onClick={handleReset} title="Start over" aria-label="Reset conversation" className="rounded-lg p-1.5 text-sky-200 transition hover:bg-white/15 hover:text-white">
                <RefreshCw className="h-4 w-4" />
              </button>
              <button onClick={() => setOpen(false)} aria-label="Close chat" className="rounded-lg p-1.5 text-sky-200 transition hover:bg-white/15 hover:text-white">
                <X className="h-4 w-4" />
              </button>
            </div>

            <div className="flex-shrink-0 py-2 text-center">
              <span className="rounded-full bg-slate-200/80 px-3 py-1 text-[11px] font-medium text-slate-500">Today</span>
            </div>
            ...
            <AnimatePresence>
              {options.length > 0 && !isTyping && (
                ...
                >
                  <p className="mb-2.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Choose a reply</p>
                  <div className="flex flex-wrap gap-2">
                    {options.map((opt) => (
                      <QuickReply key={opt.next} label={opt.label} onClick={() => handleOption(opt)} />
                    ))}
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex-shrink-0 border-t border-slate-100 bg-white py-2 text-center">
              <p className="text-[10px] text-slate-400">🏥 Sri D.K.K. Multispeciality Hospital · Kanchipuram</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
