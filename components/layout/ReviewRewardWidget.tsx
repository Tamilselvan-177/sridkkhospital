...
const GOOGLE_REVIEW_URL = "https://www.google.com/search?q=dkk+hospital+kanchipuram&oq=dkk+hospital"
    ...
    >
      ...
      <div className="relative p-5">
        ...
        {!showForm ? (
          <>
            ...
            <h3 className="mb-1 text-lg font-extrabold text-sky-950">Share Your Experience & Save! ⭐</h3>
      ...
      {!showForm && (
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/80 px-5 py-3">
          ...
          <span className="text-[10px] font-medium text-slate-500">Valid for 30 Days</span>
        </div>
      )}
    </motion.div>
  )
...
function ReviewMinimizedIcon({ onMaximize }: { onMaximize: () => void }) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      onClick={onMaximize}
      className="group relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border border-sky-100 bg-white shadow-xl"
...
function ReviewSteps() {
  return (
    <div className="mb-5 space-y-2.5 text-[11px] font-medium text-slate-600">
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <motion.a
        ...
        transition={{ duration: 10, repeat: Infinity, times: [0, 0.9, 0.95, 1] }}
      >
...
function ReviewSubmitForm({ onBack }: { onBack: () => void }) {
  const [form, setForm] = useState<{ name: string; phone: string; screenshot: File | null }>({ name: '', phone: '', screenshot: null })
  ...
  if (status === 'done') {
    return (
      <div className="flex flex-col items-center gap-3 py-6 text-center">
        ...
        <p className="mb-2 text-xs text-slate-600">Check WhatsApp — your reward is on its way!</p>
        <button
          onClick={onBack}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <div className="mb-1 flex items-center gap-2">
        ...
        <p className="text-xs font-extrabold uppercase tracking-widest text-sky-600">Claim Your Reward</p>
      </div>

      <p className="mb-1 text-[11px] leading-relaxed text-slate-600">
      ...
      >
        ...
        <span className="max-w-[200px] truncate">{form.screenshot ? form.screenshot.name : 'Upload review screenshot *'}</span>
      </button>
      <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleFile} />
      {preview && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={preview} alt="Preview" className="h-20 w-full rounded-xl border border-sky-100 object-cover" />
      )}
