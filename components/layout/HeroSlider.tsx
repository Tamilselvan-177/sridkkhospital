...
const slides = [
  ...
  },
  {
    ...
    headline: 'Advanced Medical Care for Your Family',
    subheadline: 'Providing world-class medical facilities and expert doctors right here in Kanchipuram.',
    urgency: '📅 Book your appointment today',
    urgencyColor: 'text-blue-300',
    primaryCta: { label: 'Book Appointment', path: '/contact#appointment' },
    secondaryCta: { label: 'Call Now', path: `tel:${siteData.contact.phone.replace(/\s+/g, '')}` },
    trust: [
      { icon: Star, text: '98% Patient Satisfaction' },
  ...
  {
    id: 5,
    badge: '👨‍⚕️ Expert Doctors',
    badgeColor: 'bg-red-500/90 text-white',
    headline: 'Specialized Treatment You Can Trust',
    subheadline: 'Our team of experienced specialists is dedicated to providing the best healthcare services.',
    urgency: '📞 24/7 Emergency Support',
        ...
        <div className="grid items-center gap-10 md:grid-cols-2">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="space-y-6"
            >
              <span className={`inline-flex items-center rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-widest ${slide.badgeColor} shadow-lg`}>
                {slide.badge}
              </span>
              <h1 className="text-4xl font-extrabold leading-tight text-white md:text-5xl lg:text-6xl">
                {slide.headline}
              </h1>
              <p className="max-w-xl text-base leading-relaxed text-white/80 md:text-lg">{slide.subheadline}</p>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="overflow-hidden rounded-3xl border border-white/20 shadow-2xl shadow-black/40">
                ...
                <img src={slide.image} alt={slide.imageAlt} className="aspect-[4/3] w-full object-cover" />
              </div>
              <motion.div
                animate={{ y: [0, -8, 0] }}
