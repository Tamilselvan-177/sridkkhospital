...
function ReviewSubmitForm({ onClose }: { onClose: () => void }) {
    ...
    return (
      <div className="flex flex-col items-center gap-3 py-4 text-center">
        ...
        <p className="text-sm text-slate-600">
          Check WhatsApp — your discount code <span className="font-bold text-emerald-600">REVIEW10</span> is on its way!
        </p>
        <button
          onClick={onClose}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <p className="text-xs font-semibold uppercase tracking-widest text-amber-600">Step 2 — Claim Your Reward</p>
      <p className="text-sm text-slate-600">
        Show us your Google review screenshot and we&apos;ll send your <span className="font-bold text-slate-900">10% discount code</span> via WhatsApp instantly.
      </p>
      ...
      <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleFile} aria-label="Upload review screenshot" />
      {preview && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={preview} alt="Review screenshot preview" className="h-24 w-full rounded-xl border border-amber-200 object-cover" />
      )}
...
export function ReviewPopup() {
  ...
  const variant = useRef<string | null>(null)
      ...
      {visible && (
        ...
        >
          ...
          <div className="absolute right-10 top-2 flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-400 to-orange-400 px-2.5 py-0.5">
            ...
            <span className="text-[10px] font-extrabold uppercase tracking-wide text-white">{rewardBadge}</span>
          </div>
          <button
            onClick={dismiss}
          ...
          <div className="p-5">
            {step === 'offer' ? (
              <>
                ...
                <h3 className="mb-1 text-base font-extrabold leading-snug text-slate-900">Share Your Experience &amp; Save! ⭐</h3>
                <p className="mb-1 text-[13px] leading-relaxed text-slate-600">
                  Leave us a Google review and <span className="font-bold text-slate-800">{rewardLabel}</span> at Sri D.K.K. Multispeciality Hospital, Kanchipuram.
                </p>
