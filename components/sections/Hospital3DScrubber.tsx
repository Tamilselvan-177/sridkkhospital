...
export function Hospital3DScrubber() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const frameIndex = Math.max(0, Math.min(Math.round(next), TOTAL_FRAMES - 1))
  ...
  useEffect(() => {
    const handleResize = () => {
      lastDrawnFrameRef.current = -1
      const frameIndex = Math.max(0, Math.min(Math.round(currentFrameRef.current), TOTAL_FRAMES - 1))
      drawFrame(frameIndex)
    }
    ...
    <>
      {isInView && (
        ...
        >
            ...
            {!isReady && (
              ...
              >
                <motion.div
                  ...
                  className="mb-8"
                >
                  <svg className="h-16 w-16 text-white/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0012 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75z" />
                  </svg>
                </motion.div>
                <p className="mb-4 text-lg font-semibold tracking-wide text-white/90">Loading Hospital Walkthrough…</p>
                ...
                <p className="mt-3 text-sm font-medium text-white/60">{loadingPercent}%</p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0" style={{ height: '40%' }}>
                ...
                >
                  ...
                  <div className="mt-6 flex items-center gap-2">
                    {OVERLAY_SECTIONS.map((_, i) => (
                      <div
                        key={i}
                        className={`h-2 rounded-full transition-all duration-500 ${i === activeSection ? 'w-8 bg-white' : 'w-2 bg-white/40'}`}
                      />
                    ))}
                    <span className="ml-3 text-xs font-medium uppercase tracking-wider text-white/50">
          ...
          {isReady && scrollProgress < 0.05 && (
            ...
            >
              ...
              >
                <span className="mb-2 text-xs font-semibold uppercase tracking-widest text-white/70">Scroll to explore</span>
      ...
      <div ref={containerRef} style={{ height: '300vh' }} className="relative bg-slate-950" />
    </>
  )
}
