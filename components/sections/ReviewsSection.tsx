...
function ReviewCard({ review, index }: { review: ReviewType; index: number }) {
  return (
    <Reveal direction={index % 2 === 0 ? 'left' : 'right'}>
      <div className="relative h-full rounded-2xl border border-amber-100 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-sm font-bold text-white ${review.avatarColor}`}>
              {review.avatar}
            </div>
            <div>
  ...
  return (
    <section className="container-shell rounded-[2rem] bg-gradient-to-b from-amber-50/60 to-white py-14">
      ...
      <Reveal className="mx-auto mb-10 max-w-md">
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-amber-200 bg-white px-6 py-5 text-center shadow-sm">
      ...
      <div className="mt-10 text-center">
        <p className="mb-4 text-sm text-slate-600">Treated at SRI DKK? Your review helps other families find trusted care.</p>
        <a
          href={siteData.contact.reviewsUrl}
          target="_blank"
          rel="noopener noreferrer"
