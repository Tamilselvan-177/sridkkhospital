...
export type Transformation = typeof transformations[0]
      ...
      <div className="absolute inset-0 overflow-hidden" style={{ width: `${pos}%` }}>
        <div
          ...
          style={{
            ...
            backgroundRepeat: 'no-repeat',
            width: containerRef.current?.offsetWidth ? `${containerRef.current.offsetWidth}px` : '100vw',
          }}
        />
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4 pt-2">
        ...
        <h3 className="text-sm font-bold leading-snug text-slate-900">{item.treatment}</h3>
        <p className="text-xs leading-relaxed italic text-slate-600">&ldquo;{item.caption}&rdquo;</p>

        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl bg-red-50 p-2.5">
            <p className="mb-1 text-[9px] font-bold uppercase tracking-widest text-red-500">Before</p>
            <p className="text-xs leading-snug text-slate-700">{item.beforeDesc}</p>
          </div>
          <div className="rounded-xl bg-emerald-50 p-2.5">
            <p className="mb-1 text-[9px] font-bold uppercase tracking-widest text-emerald-600">After</p>
            <p className="text-xs leading-snug text-slate-700">{item.afterDesc}</p>
          </div>
        </div>
...
export function TransformationsSection() {
  ...
  function setFilter(f: string) { setActiveFilter(f); setPage(0) }
  ...
  return (
    <section className="container-shell rounded-[2rem] bg-gradient-to-b from-slate-50 to-white py-14">
      ...
      <Reveal className="mt-10 rounded-3xl bg-gradient-to-r from-sky-900 via-sky-800 to-indigo-900 p-8 text-center text-white">
        <p className="mb-1 text-xs font-bold uppercase tracking-widest text-sky-300">Your Transformation Awaits</p>
        <h3 className="mb-3 text-2xl font-bold">Ready to see your results?</h3>
