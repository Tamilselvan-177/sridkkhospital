...
const CATEGORIES = [
  { id: 'dental', label: '🦷 Dental', slugs: ['root-canal-treatment', 'dental-implants', 'dentures', 'orthodontic-treatments', 'tooth-extraction', 'smile-designing'] },
  { id: 'aesthetics', label: '✨ Medical Aesthetics', slugs: ['botox', 'fillers', 'laser-hair-removal', 'skin-lightening-treatments'] },
  { id: 'hair', label: '💆 Hair Restoration', slugs: ['hair-prp', 'hair-gfc'] },
]
...
export default function TreatmentsPageClient({ treatments }: { treatments: Treatment[] }) {
  const [activeTab, setActiveTab] = useState('all')

  const visibleTreatments =
    activeTab === 'all'
      ? treatments
      : treatments.filter((t) => CATEGORIES.find((c) => c.id === activeTab)?.slugs.includes(t.slug))
        ...
        <div className="container-shell text-center">
          <Reveal>
            <p className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-sky-200">
              <Stethoscope className="h-3.5 w-3.5" /> Evidence-Based Treatments
            </p>
          ...
          <Reveal className="mt-10 flex flex-wrap justify-center gap-4">
            {[
              ...
              >
                <Icon className="h-4 w-4 text-sky-300" /> {text}
              </div>
            ))}
          </Reveal>
        </div>
      ...
      <section className="container-shell py-10">
        <div className="mb-8 flex flex-wrap justify-center gap-2">
          <button
            ...
            className={`rounded-full px-5 py-2 text-sm font-semibold transition ${activeTab === 'all' ? 'bg-sky-700 text-white shadow-md' : 'border border-slate-200 bg-white text-slate-600 hover:border-sky-300 hover:text-sky-700'}`}
          >
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`rounded-full px-5 py-2 text-sm font-semibold transition ${activeTab === cat.id ? 'bg-sky-700 text-white shadow-md' : 'border border-slate-200 bg-white text-slate-600 hover:border-sky-300 hover:text-sky-700'}`}
            >
              {cat.label}
            </button>
          ))}
      ...
      <section className="container-shell pb-14">
        <Reveal className="rounded-3xl bg-gradient-to-r from-emerald-700 to-teal-700 p-8 text-center text-white shadow-xl">
          ...
          <p className="mx-auto mb-6 max-w-lg text-sm text-emerald-100">
            Our specialists will assess your condition honestly and recommend only what genuinely helps.
            No pressure, no unnecessary procedures.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <Link
              href="/contact#appointment"
            ...
            <a
              href={toWhatsAppLink(siteData.contact.whatsapp, 'Hi! I need guidance on which treatment is right for my condition. Can we discuss?')}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-6 py-3 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/20"
            >
