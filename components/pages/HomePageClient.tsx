...
import { Timer, PlaySquare } from 'lucide-react'
...
const Hospital3DScrubber = dynamic(
  () => import('@/components/sections/Hospital3DScrubber').then((m) => ({ default: m.Hospital3DScrubber })),
  { ssr: false },
)
...
const TransformationsSection = dynamic(
  () => import('@/components/sections/TransformationsSection').then((m) => ({ default: m.TransformationsSection })),
  { ssr: false },
)

interface ModalState {
  item: { name: string; image: string; description?: string; benefits?: string[]; duration?: string; badge?: string; highlight?: string }
  type: 'treatment' | 'specialty'
}

interface HomePageClientProps {
  treatments: Array<{ slug: string; name: string; image: string; description: string; benefits: string[]; duration: string; badge?: string | null; highlight?: string | null; category: string }>
  specialties: Array<{ slug: string; name: string; image: string; description: string; benefits: string[]; duration: string; badge?: string | null; highlight?: string | null }>
  blogs: Array<{ slug: string; title: string; image: string }>
  siteData: typeof SiteDataType
}

export default function HomePageClient({ treatments, specialties, blogs, siteData }: HomePageClientProps) {
